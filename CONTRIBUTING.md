# Contributing to korea-scholarship-mcp
...
```bash
git clone https://github.com/ckgerteis/korea-scholarship-mcp
cd korea-scholarship-mcp
python -m venv .venv
...
`python mcpb/build.py` writes `dist/korea-scholarship-mcp-<version>.mcpb`. The bundle vendors nothing: its manifest
...
the install step and download everything inside the launch timeout, which is the fault 0.6.2 repaired.
